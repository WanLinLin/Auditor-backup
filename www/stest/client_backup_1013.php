...
/* INCLUDE RHYME */
$sql = "(SELECT * FROM (SELECT text FROM `word` join `tag` using (wId) join `rhyme` using (wId) WHERE rhymes = '$rhyme' and text like '$sql_num' and tags = '$tag' and link >= 1 $sql_andTags ORDER BY link DESC limit 5) text)
     UNION
     (SELECT * FROM (SELECT text FROM `word` join `tag` using (wId) join `rhyme` using (wId) WHERE rhymes = '$rhyme' and text like '$sql_num' and tags = '$tag' and link >= 1 ORDER BY link DESC limit 4) text)
     UNION
     (SELECT * FROM (SELECT text FROM `word` join `tag` using (wId) join `rhyme` using (wId) WHERE rhymes = '$rhyme' and text like '$sql_num' and tags = '$tag' and link < 1 $sql_andTags GROUP BY wId ORDER BY distance DESC limit 5) text)
     UNION
     (SELECT * FROM (SELECT text FROM `word` join `tag` using (wId) join `rhyme` using (wId) WHERE rhymes = '$rhyme' and text like '$sql_num' and tags in ($sql_orTags) ORDER BY distance DESC limit 5) text)
     UNION
     (SELECT * FROM (SELECT text FROM `word` join `rhyme` using (wId) WHERE text like '$sql_num' and rhymes = '$rhyme' ORDER BY rand() limit 5) text)
     UNION
     (SELECT * FROM (SELECT text FROM `word` join `rhyme` using (wId) WHERE text like '$sql_num' ORDER BY rand() limit 5) text) limit 5";
